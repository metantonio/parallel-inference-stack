...
# Configuration for batching simulation
...
async def process_batch(batch_items: List[Dict[str, Any]]):
    """
    Simulate vLLM's dynamic batching by processing multiple requests together.
    This simulates GPU batching where multiple requests are processed in parallel.
    """
    ...
    async with batch_semaphore:
        ...
        # In real vLLM, this would be actual GPU batching
        # Processing time is slightly longer than single request but much faster than sequential
        base_time = 0.5  # Base processing time
        batch_overhead = 0.1 * batch_size  # Small overhead per item in batch
        ...
        for item in batch_items:
            ...
            if "messages" in request:
                ...
                response_text = f"[Batched response {batch_id[:8]}] Mock response to: {content}"
            else:
                response_text = f"[Batched response {batch_id[:8]}] Mock completion for: {request.get('prompt', '')}"
...
async def startup_event():
    ...
    print("=" * 70)
    print("🚀 Mock vLLM Server with Dynamic Batching Started")
    print("=" * 70)
    ...
    print(f"   - Requests are automatically batched for GPU efficiency")
